...
public class TCPServerTest5 : MonoBehaviour
{
            ...
            while (stream.DataAvailable)
            {
                ...
                if (r >= 10)
                {
                    if(vs[0] == 0)
                    {
                        ...
                        {
                            DisplayClient wkdispClient = displayClientManager.getDisplayClient(dispid);
                            if (wkdispClient.UdpClient.Client.SendBufferSize > udpbufsize)
                            {
                                wkdispClient.UdpClient.Client.SendBufferSize = udpbufsize;
                            }
                            NetworkStream ns = wkdispClient.TcpClient.GetStream();
    ...
    public class DisplayClientManager
    {
        ...
        {
            DisplayClient dc = displayClientList.Find(d => d.DispId == dispID);
        ...
        {
            DisplayClient dc = displayClientList.Find(d => d.Ip == ip && d.UdpPort == udpport);
            if (dc == null)
            {
    ...
    public class DisplayClient
    {
        ...
        public int TcpPort
        {
            get
            {
                ...
                else return ((IPEndPoint)TcpClient.Client.RemoteEndPoint).Port;
            }
        }
        public int UdpPort
        {
        ...
        public DisplayClient(int dispID, String ip, int tcpPort, int udpPort)
        {
            try
            {
                ...
                this.TcpClient = new TcpClient(ip, tcpPort);
            }
            catch(Exception e)
            {
                Debug.Log(e);
